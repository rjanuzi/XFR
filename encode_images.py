...
def main():
    ...
    (
        ...
        Gs_network,
    ) = pretrained_networks.load_networks(args.model_url)

    generator = Generator(
        Gs_network, args.batch_size, randomize_noise=args.randomize_noise
    )
    if args.dlatent_avg != "":
    ...
    for images_batch in tqdm(
        ...
        for img_array, dlatent, img_path, img_name in zip(
            ...
            img.save(
                os.path.join(
                    args.generated_images_dir, f"{img_name}_test_no_masks.png"
                ),
                "PNG",
            )
            np.save(
                os.path.join(args.dlatent_dir, f"{img_name}_test_no_masks.npy"), dlatent
            )
        ...
        for img_array, dlatent, img_path, img_name in zip(
            ...
            img = PIL.Image.fromarray(img_array, "RGB")
            img.save(
                os.path.join(args.generated_images_dir, f"{img_name}_test_masks.png"),
                "PNG",
            )
            np.save(
                os.path.join(args.dlatent_dir, f"{img_name}_test_masks.npy"), dlatent
            )
